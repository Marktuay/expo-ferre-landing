...
- **Integración de Plano Interactivo de Stands en la Landing Page (`App.jsx` & `InteractiveMap.jsx`):**
  ...
  - **Ampliación de Logotipos al Hacer Hover:** Al pasar el cursor sobre cualquier globo de stand en el mapa, el pin se amplía (`scale-125`) y despliega una tarjeta flotante emergente con una vista del logotipo mucho más grande (hasta `max-h-[170px] max-w-[280px]`) en alta definición sin mostrar el valor numérico del stand.
  - **Ocultamiento de Precio en Hover:** Se removió la indicación del precio/valor en dinero del cuadro flotante del mapa para mantener enfocada la tarjeta en la marca, nombre y dimensiones del stand.
  - **Distribución de Columnas 40% / 60%:** Se ajustó la sección de la portada para otorgar un **40% de ancho a la columna de texto descriptivo** (izquierda) y un **60% de ancho al mapa interactivo** (derecha) en pantallas de escritorio.
