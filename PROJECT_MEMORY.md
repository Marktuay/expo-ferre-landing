...
- **Integración de Plano Interactivo de Stands en la Landing Page (`App.jsx` & `InteractiveMap.jsx`):**
  ...
  - **Ampliación de Logotipos al Hacer Hover:** Al pasar el cursor sobre cualquier globo de stand en el mapa, el pin se amplía (`scale-125`) y despliega una tarjeta flotante emergente con una vista del logotipo mucho más grande (hasta `max-h-[170px] max-w-[280px]`) en alta definición. Se aplicó también un efecto de amplificación (`hover:scale-110`) a las tarjetas del carrusel de marcas en la portada.
