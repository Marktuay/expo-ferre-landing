...
const GuestForm = ({ onBack }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, `${getEventBasePath()}/guests`), data);
      
      setRegisteredGuestId(docRef.id);
        ...
        <div className="bg-white p-8 md:p-12 rounded-lg shadow-sm border border-outline-variant">
          {formState === 'success' ? (
            <div className="bg-white p-8 rounded-lg border border-outline-variant text-center flex flex-col items-center gap-6">
              ...
              <div>
                <h3 className="font-bold text-2xl mb-2 text-primary">¡Invitado registrado con éxito!</h3>
                <p className="text-secondary mb-6">El código QR para el acceso de su invitado ha sido generado.</p>
                
                <div className="bg-surface-variant p-6 rounded-lg inline-block border border-outline mb-6">
                  <QRCodeSVG value={registeredGuestId} size={180} level="M" />
                  <p className="mt-4 text-sm font-mono text-secondary">ID: {registeredGuestId}</p>
                </div>
