...
const OFFICIAL_SPONSORS = [
  ...
  { company: 'Nitrotel', category: 'Diamante', logo: '/diamante/nitrotel.png' },
  ...
  { company: 'Indenicsa', category: 'Diamante', logo: '/diamante/indeninicsa.png' },
  { company: 'ArcelorMittal', category: 'Diamante', logo: '/diamante/LOGO-ARCELOR.png' },
  { company: 'Romax', category: 'Diamante', logo: '/diamante/romax.jpeg' },
  { company: 'Maximiza', category: 'Diamante', logo: '/diamante/maximiza.jpeg' },
...
export default function AdminSponsors({ onBack }) {
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7] p-4 md:p-8 pt-40 md:pt-48">
      <div className="w-full max-w-[98%] xl:max-w-[1500px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          ...
          <div className="flex flex-wrap gap-2 md:gap-3">
            <button 
              ...
              className="px-4 py-2 bg-secondary text-white border border-secondary rounded-md hover:brightness-110 transition-colors font-label-lg flex items-center gap-1.5 text-sm"
            >
            ...
            <button 
              ...
              className="px-4 py-2 bg-primary text-on-primary border border-primary rounded-md hover:brightness-110 transition-colors font-label-lg flex items-center gap-1.5 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            }} className="px-4 py-2 bg-[#217346] text-white border border-[#217346] rounded-md hover:brightness-110 transition-colors font-label-lg flex items-center gap-1.5 text-sm">
              <span className="material-symbols-outlined text-lg">download</span>
              Exportar Excel
            </button>
            <button onClick={onBack} className="px-4 py-2 bg-surface text-on-surface border border-outline-variant rounded-md hover:bg-surface-variant transition-colors font-label-lg flex items-center gap-1.5 text-sm">
        ...
        <div className="bg-white rounded-lg shadow-md border border-outline-variant overflow-hidden w-full">
          <div className="w-full overflow-hidden">
            <table className="w-full text-left border-collapse text-xs md:text-sm">
              <thead>
                <tr className="bg-surface-variant/30 border-b border-outline-variant">
                  <th className="px-3 py-3 font-bold text-on-surface">Nombre</th>
                  <th className="px-3 py-3 font-bold text-on-surface">Empresa</th>
                  <th className="px-3 py-3 font-bold text-on-surface">Stand</th>
                  <th className="px-3 py-3 font-bold text-on-surface">Email / Rol</th>
                  <th className="px-3 py-3 font-bold text-on-surface">Teléfono</th>
                  <th className="px-3 py-3 font-bold text-on-surface text-center">Emp.</th>
                  <th className="px-3 py-3 font-bold text-on-surface text-center">Estado</th>
                  <th className="px-3 py-3 font-bold text-on-surface text-center">Fecha</th>
                  <th className="px-3 py-3 font-bold text-on-surface text-center">Acciones</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  sponsors.map((sponsor) => (
                    <tr key={sponsor.id} className="border-b border-outline-variant hover:bg-surface-variant/10 transition-colors">
                      <td className="px-3 py-2.5 text-on-surface font-medium">
                        <div className="flex items-center gap-1.5">
                          {sponsor.lastActive && (Date.now() - sponsor.lastActive.toMillis() < 5 * 60 * 1000) ? (
                            <span className="w-2 h-2 bg-green-500 rounded-full shrink-0" title="En línea"></span>
                          ) : (
                            <span className="w-2 h-2 bg-gray-300 rounded-full shrink-0" title="Desconectado"></span>
                          )}
                          <span className="truncate max-w-[130px] inline-block" title={(`${sponsor.nombre || sponsor.name || ''} ${sponsor.apellido || ''}`).trim() || 'Sin Nombre'}>
                            {(`${sponsor.nombre || sponsor.name || ''} ${sponsor.apellido || ''}`).trim() || 'Sin Nombre'}
                          </span>
                        </div>
                      </td>
                      <td className="px-3 py-2.5 text-secondary font-semibold">
                        <span className="truncate max-w-[120px] inline-block" title={sponsor.empresa || sponsor.company || 'N/A'}>
                          {sponsor.empresa || sponsor.company || 'N/A'}
                        </span>
                      </td>
                          ...
                          sponsor.standList.length <= 2 ? (
                            <div className="flex flex-wrap gap-1">
                              {sponsor.standList.map(name => (
                                <span key={name} className="bg-blue-100 text-blue-800 text-[10px] md:text-xs font-bold px-2 py-0.5 rounded-full border border-blue-300 whitespace-nowrap">{name}</span>
                              ))}
                            </div>
                          ) : (
                            <div className="flex items-center gap-1" title={sponsor.standList.join(', ')}>
                              <span className="bg-blue-100 text-blue-800 text-[10px] md:text-xs font-bold px-2 py-0.5 rounded-full border border-blue-300 whitespace-nowrap">
                                {sponsor.standList[0]}
                              </span>
                              <span className="bg-gray-100 text-gray-700 text-[10px] md:text-xs font-bold px-1.5 py-0.5 rounded-full border border-gray-300 whitespace-nowrap">
                                +{sponsor.standList.length - 1}
                              </span>
                            </div>
                          )
                        ) : (
                          <span className="bg-gray-100 text-gray-500 text-[10px] md:text-xs font-medium px-2 py-0.5 rounded-full border border-gray-200 whitespace-nowrap">Sin Stand</span>
                        )}
                      </td>
                      <td className="px-3 py-2.5 text-secondary">
                        {sponsor.isOfficial || sponsor.correo === 'Patrocinador Oficial' ? (
                          <span className="bg-amber-100 text-amber-900 border border-amber-300 text-[10px] md:text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap">Oficial</span>
                        ) : (
                          <span className="truncate max-w-[140px] inline-block" title={sponsor.correo || sponsor.email || 'N/A'}>
                            {sponsor.correo || sponsor.email || 'N/A'}
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-2.5 text-secondary text-xs whitespace-nowrap">{sponsor.telefono || sponsor.phone || 'N/A'}</td>
                      <td className="px-3 py-2.5 text-secondary text-center text-xs whitespace-nowrap">{sponsor.empleados || 'N/A'}</td>
                      <td className="px-3 py-2.5 text-center whitespace-nowrap">
                        {(!sponsor.status || sponsor.status === 'approved') ? (
                          <span className="bg-green-100 text-green-800 text-[10px] md:text-xs font-medium px-2 py-0.5 rounded-full border border-green-400">Aprobado</span>
                        ) : (
                          <span className="bg-yellow-100 text-yellow-800 text-[10px] md:text-xs font-medium px-2 py-0.5 rounded-full border border-yellow-400">Pendiente</span>
                        )}
                      </td>
                      <td className="px-3 py-2.5 text-secondary text-center text-xs whitespace-nowrap">
                        {sponsor.isOfficial ? (
                          <span className="text-[10px] md:text-xs font-medium text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full border border-gray-200 whitespace-nowrap">Confirmado</span>
                        ) : (
